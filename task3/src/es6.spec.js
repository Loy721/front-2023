...
describe('es6', () => {
    ...
    describe('#Dictionary', () => {
        it('корректное создание экземпляра', () => {
            const dic = new core.Dictionary();
            assert.strictEqual(!!dic, true);
        });
        it('добавление и извлечение элементов', () => {
            const dic = new core.Dictionary();
            dic.add('1', '2')
            assert.strictEqual(dic.getValue('1'), '2');
            assert.strictEqual(dic.getValue('wsda'), 'Такого ключа не существует');
        });
    });
});